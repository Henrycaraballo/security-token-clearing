...
import { Address, SecurityToken } from "../src/Types";
...
const upgradeValue = <A>(x: A | null, f: (y: A) => void): void => {
  if (x === null) {
...
describe("PolyMath interface", async () => {
  ...
  describe("deployPolymath", () => {
    it("should deploy a whitelisted token", async function() {
      const tokenAddress = await deployPolymath(
        this.controller,
        this.exchange,
        "regs",
        web3
  ...
  describe("putInvestor", () => {
    before(async function() {
      ...
      this.tokenAddress = await deployPolymath(
        this.controller,
        this.exchange,
        "regs",
        web3
    ...
    it("should accurately install an investor", async function() {
      const transcript = await putInvestor(
        this.investor1,
        this.investorAddress1,
        this.tokenAddress,
        this.cWeb3
      );
      ...
      upgradeValue(whitelistedInvestor, whitelistedInvestor => {
        assert.equal(whitelistedInvestor.addedBy, this.controller);
        assert(
          moment(whitelistedInvestor.expiry).isSame(
            moment().add(3, "months"),
            "days"
          )
        );
        assert(
          moment(whitelistedInvestor.from).isSame(
            moment().add(1, "years"),
            "days"
          )
        );
    ...
    it("should be idempotent, does not update on multiple calls", async function() {
      let whitelistedInvestor = await getInvestorFromWhitelist(
        this.transferManager,
        this.investorAddress1
      );
      upgradeValue(whitelistedInvestor, whitelistedInvestor => {
    ...
    context("current KYC is in the past", async function() {
      ...
      it("should update KYC validity date to 3 months from now", async function() {
        ...
        whitelistedInvestor = await getInvestorFromWhitelist(
          this.transferManager,
          this.investorAddress1
        );
        upgradeValue(whitelistedInvestor, whitelistedInvestor => {
      ...
      context("the investor has been whitelisted previously", async function() {
        ...
        it("should set investor to not be able to trade", async function() {
          ...
          upgradeValue(whitelistedInvestor, whitelistedInvestor => {
            ...
            assert(
              moment(whitelistedInvestor.to).isSame(
                moment().subtract(1, "years"),
                "days"
              )
            );
